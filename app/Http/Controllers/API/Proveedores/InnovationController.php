...
class InnovationController extends Controller
{
    ...
    public function v2()
    {
        ...
        for ($i = 1; $i <= $pag;$i++ ) {
            ...
            foreach ($response['data'] as $key => $value) 
            {
                $prev_product = Product::where('parent_code',$value['codigo'])->first();
                if ($prev_product == null) {
                    $this->insertProduct($value, $cont_new_products);
                } else {
                    $this->updateProduct($value, $cont_update_products);
                }
            }
        }
        ...
        return response()->json([
            'status' => 'success',
            'msg' => 'Se agregaron '.$msg.' de Innovation'
        ]);
    ...
    private function insertProduct($item, &$cont_new_products) {
        ...
        $product->parent_code = $item['codigo'];
        #Seccion colores e Imagenes
        $images = [];
        $colores = [];
        $printing_methods = [];

        if (array_key_exists('imagen_principal', $item) && !empty($item['imagen_principal'])) {
            array_push($images, $item['imagen_principal']);
        }

        if (array_key_exists('images', $item) && !empty($item['images'])) {
            foreach ($item['images'] as $key => $img) {
                array_push($images, $img['image']);
            }
        }

        if (array_key_exists('imagenes_adicionales', $item) && !empty($item['imagenes_adicionales'])) {
            foreach ($item['imagenes_adicionales'] as $key => $img) {
                array_push($images, $img);
            }
        }
        
        #Verificamos si existe colores en el array y si no viene vacio
        if (array_key_exists('colores', $item) && !empty($item['colores'])) {
            foreach ($item['colores'] as $key => $color) {
        ...
        $product->nw = $item['peso_paquete'];
        $product->weight_product = $item['peso_producto'];
        $product->printing_area = $item['area_impresion'];

        #Verificamos si existe tipos de impresiony si en el array viene vacio
        if (array_key_exists('tecnicas_impresion', $item) && !empty($item['tecnicas_impresion'])) {
            foreach ($item['tecnicas_impresion'] as $key => $impresion) {
                array_push($printing_methods, $impresion['codigo']);
            }
        }
        $product->printing_methods = json_decode($printing_methods);

        if (!empty($item['categorias']['categorias'])) {
            
            $product->category = $item['categorias']['categorias']['nombre'];
        }

        if (!empty($item['categorias']['subcategorias'])) {
            
            $product->subcategory = $item['categorias']['subcategorias']['nombre'];
        }
        $product->box_pieces = $item['cantidad_por_paquete'];
        $product->material = Str::upper($item['material']);
        $product->proveedor = 'Innovation';
        $product->save();

        $meta_key = new MetaKey();
        $meta_key->keyword = $item['meta_description'];
        $meta_key->save();
