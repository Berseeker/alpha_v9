    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->producto_id as $key => $id) {
            ...
            if ($product != null) {
                ...
                $order_x_product->pantone = (!empty($request->pantone) && isset($request->pantone[$key])) ? $request->pantone[$key] : 'Sin definir';
                $order_x_product->num_ink = (!empty($request->no_ink) && isset($request->no_ink[$key])) ? (int) $request->no_ink[$key] : 1;
                $order_x_product->num_pzas = (!empty($request->noPzas) && isset($request->noPzas[$key])) ? $request->noPzas[$key] : 100;
                $order_x_product->printing_method = (!empty($request->printing_methods) && isset($request->printing_methods[$key])) ? $request->printing_methods[$key] : 'Sin definir';
